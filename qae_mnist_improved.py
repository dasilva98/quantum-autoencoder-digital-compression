...
print("x_train size after cut: ", len(x_train))
...
print("Deleting no valuable information for the training set...")
k = 0
...
print("Deleting no valuable information for the test set...")
k = 0
...
print("Normalizing the training set data...")
for i in range(len(x_train)):
    x_train[i] = Normalize(x_train[i])
print("Normalizing the test set data...")
...
print("Combining the initial circuit, the quantum variatinal circuit and the swap test...")
...
circuit_full.draw(output="mpl", style="iqp")

#We generate the inverse of the QVC
vqc(n,num_layers,params).inverse().draw(output = "mpl", style="iqp")
...
for i in range(1):
    ...
    print("Minimizing of the objective_fucntion by a COBYLA method...")
    ...
    print("cost of the solution array: ",objective_function(params))
    print("solution array: ", params)
...
print("Compressing the test set values...")
...
print("Compressing the training set values...")
...
# use the created array to output your multiple images. In this case I have stacked 4 images vertically
axarr[0].imshow(np.asarray(decoder[0].real).reshape(8,8)*255)
axarr[1].imshow(np.asarray(decoder[1].real).reshape(8,8)*255)
axarr[2].imshow(np.asarray(decoder[2].real).reshape(8,8)*255)
axarr[3].imshow(np.asarray(decoder[3].real).reshape(8,8)*255)
axarr[4].imshow(np.asarray(decoder[4].real).reshape(8,8)*255)
plt.show()
...
print("Calculating the MSE, PSNR and SSIM between the reshaped decoded image and the trainig set image...")
for i in range(len(x_train)):
    ...
    decoded_img = np.asarray(decoder_train[i].real).reshape(8,8)*255 
    ...
    ssim_list.append(ssim(decoded_img.astype("float"),test_img.astype("float"), data_range=test_img.max() - test_img.min()))
    mse_list.append(Y) #Save the MSE value
    aux = (64**2)/Y #Calculate the PSNR
    psnr_list.append(10*math.log10(aux)) #Save the PSRN value

print("Plotting the MSE, PSNR and SSIM results of the training set")
...
print("Calculating the MSE, PSNR and SSIM between the reshaped decoded image and the test set image...")
for i in range(len(x_test)):
    #Reshape to a 8X8 image of the test set
    test_img = x_test[i].reshape(8,8)*255
    #Reshape to a 8X8 image of the decoded test set
    decoded_img = np.asarray(decoder[i].real).reshape(8,8)*255
...
print("Plotting the MSE, PSNR and SSIM results of the test set")
from matplotlib import pyplot as plt
plt.plot(mse_list)
plt.show()
